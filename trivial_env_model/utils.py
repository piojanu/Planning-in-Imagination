...
def eval_in_batches(model, dataset, reward_loss_factor=0.01, eval_size=1000):
    ...
    for i in range(num_batches):
        states_batch, next_states_batch, actions_batch, rewards_batch = dataset.get_next_batch(
            eval_size)
        states_pred, rewards_pred = model(torch.Tensor(
            states_batch), torch.Tensor(actions_batch))
        loss = F.mse_loss(states_pred, torch.Tensor(next_states_batch))
        rewards_loss = reward_loss_factor * \
            F.mse_loss(rewards_pred, torch.Tensor(rewards_batch))
...
def test_model(model, dataset, interval=0.003):
    ...
    for i in range(dataset.num_samples):
        ...
        state_pred, reward_pred = model(
            torch.Tensor(states), torch.Tensor(actions))

        fig.add_subplot(1, 3, 1)
        plt.suptitle("Step: {}, reward_pred: {:.5f}".format(
            i, reward_pred.detach().cpu().numpy()[0][0]))
