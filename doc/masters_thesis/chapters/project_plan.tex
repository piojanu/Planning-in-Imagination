...
\For{optimization iteration $i = 1..I$}{
  ...
  \For{candidate action sequence $j = 1..J$}{
    ...
    $z^{(j)}_{t:t+H+1} \sim q(z_t|o_{\leqslant t}, a_{< t})\prod^{t+H+1}_{\tau=t+1}p(z_\tau|z_{\tau-1}, a^{(j)}_{\tau-1})$\\
    $R^{(j)} = \sum^{t+H+1}_{\tau=t+1}E[p(r_\tau|z^{(j)}_\tau)]$\\
  }
  // Re-fit belief to the $K$ best action sequences.\\
  $\kappa \leftarrow argsort \big( \big\{ R^{(j)} \big\}^J_{j=1} \big)_{1:K}$\\
  $\mu_{t:t+H} = \frac{1}{K}\sum_{k \in \kappa}a^{(j)}_{t:t+H},\text{  }\sigma_{t:t+H} = \frac{1}{K-1}\sum_{k \in \kappa}|a^{(j)}_{t:t+H} - \mu_{t:t+H}|$\\
  $q(a_{t:t+H}) \leftarrow Normal(\mu_{t:t+H}, \sigma^2_{t:t+H} \mathbb I)$
}
